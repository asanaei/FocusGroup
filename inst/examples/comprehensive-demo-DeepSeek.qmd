...
fg <- FocusGroup$new(
  ...
  admin_config = cfg_admin,
  max_tokens_config = list(utterance = 380, moderator = 500, desire = 22)
)

run_result <- tryCatch(
  {
    num_rounds_live <- suppressWarnings(as.integer(Sys.getenv("FG_DEMO_ROUNDS", "24")))
    if (is.na(num_rounds_live) || num_rounds_live < 20) num_rounds_live <- 24L
    fg$run_simulation(num_rounds = num_rounds_live, verbose = TRUE)
  },
  error = function(e) {
...
message_df <- build_message_dataframe(fg$conversation_log)
...
phase_stats <- message_df |>
  ...
  mutate(share = round(100 * messages / sum(messages), 1)) |>
  arrange(desc(messages))
kable(phase_stats, caption = "Message distribution by phase")

avg_message_length <- message_df |>
  filter(!is_moderator) |>
  summarise(
...
kable(avg_message_length, caption = "Participant message length diagnostics")
```

## Education and Party Signal Tables

```{r subgroup-signals}
message_with_meta <- message_df |>
  filter(!is_moderator) |>
...
signal_by_party <- message_with_meta |>
  mutate(
...
signal_by_education <- message_with_meta |>
  mutate(
    support_hits = str_count(text, support_pat),
    concern_hits = str_count(text, concern_pat),
    market_hits = str_count(text, market_pat),
...
theme_res <- fg$analyze_themes(config = cfg_admin)
